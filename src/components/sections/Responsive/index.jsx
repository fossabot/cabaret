import React from 'react'
import { Grid, Row, Col } from '../../Elements/Grid'

const ResponsiveSection = () => (
  <section className="landing__device-section">
    <Grid>
      <h2>Acesse o SITE de qualquer dispositivo</h2>
      <p>Acesse de onde e como quiser!</p>
      <p>Você pode levar as mais gostosas com você, acesse do seu celular, tablet, desktop ou qualquer outro dispositivo.</p>
      <p>Sua única preocupação é qual escolher!</p>
    </Grid>
  </section>
)

export default ResponsiveSection
